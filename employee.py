...
class EmployeeSystem:
    def __init__(self,root):
        self.root=root
        self.root.title("Employee Payroll Management System | Developed by Python 3.0")
        ...
        btn_emp=Button(self.root,text="All Employee's",command=self.employee_frame,font=("times new roman",13),bg="grey",fg="white").place(x=1100,y=10,height=30,width=120)
        #==========Frame1=========================
        #============Variables==================
        ...
        self.txt_code.place(x=220,y=72,width=200)
       ...
       #=======Row5
        ...
        txt_salary=Entry(Frame2,font=("times new roman",15),textvariable=self.var_salary,bg="lightyellow",fg="black").place(x=450,y=62,width=100)
        ...
        btn_clear=Button(Frame2,text="Clear",command=self.clear,font=("times new roman",20),bg="grey",fg="black").place(x=420,y=225,height=30,width=120)
...
#========================== All Function Start Here ==================================================
    ...
    def delete(self):
        if self.var_emp_code.get()=='':
            messagebox.showerror("Error","Employee Id must be reqired")
        else:
            try:
                con=pymysql.connect(host='localhost',user='root',password='',db='ems')
                cur=con.cursor()
    ...
    def add(self):
        if self.var_emp_code.get()=='' or self.var_net_salary.get()=='' or self.var_name.get()=='':
            ...
            try:
                ...
                # print(rows)
                if row!=None:
                    messagebox.showerror("Error","This Employee ID has alredy available in our record, try again with another ID",parent=self.root)
                else:
        ...
        if self.var_emp_code.get()=='' or self.var_net_salary.get()=='' or self.var_name.get()=='':
            ...
            try:
                ...
                if row==None:
                    ...
                    cur.execute("UPDATE `emp_salary` SET `designation`=%s,`name`=%s,`age`=%s,`gender`=%s,`email`=%s,`hr_location`=%s,`doj`=%s,`dob`=%s,`experience`=%s,`proof_id`=%s,`contact`=%s,`status`=%s,`address`=%s,`month`=%s,`year`=%s,`basic_salary`=%s,`t_days`=%s,`absent_days`=%s,`medical`=%s,`pf`=%s,`convence`=%s,`salary_receipt`=%s WHERE `e_id`=%s",
                    (
...
'''
    ...
    def employee_frame(self):
        ...
        self.root2.title("Employee Payroll Management System | Developed by Python 3.0")
        ...
        self.employee_tree=ttk.Treeview(self.root2,columns=('e_id', 'designation', 'name', 'age', 'gender', 'email', 'hr_location', 'doj', 'dob', 'experience', 'proof_id', 'contact', 'status', 'address' , 'month', 'year', 'basic_salary', 't_days', 'absent_days', 'medical', 'pf', 'convence', 'net_salry', 'salary_receipt'),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.employee_tree.heading('net_salry',text='Net Salary')
        ...
        self.employee_tree.column('net_salry',width=100)
